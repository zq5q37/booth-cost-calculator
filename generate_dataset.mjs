...
const baseDir = './images_webp/stickers';
...
function getSizes(filename) {
  ...
  return sizes.length ? sizes : ['Sticker']; // default if no size found
}
...
fs.readdirSync(baseDir).forEach((file) => {
  ...
  if (fs.statSync(filePath).isFile() && /\.(jpg|jpeg|png|webp|avif)$/i.test(file)) {
    dataset.push({
      image: filePath,
      sizes: getSizes(file) // your function to determine sizes
    });
  }
});
